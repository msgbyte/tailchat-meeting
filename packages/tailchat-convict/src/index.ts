...
// alias
(types as any).integer = types.int;

const custom_converters = new Map();

const parsers_registry = { '*': JSON.parse };

function flatten(obj, useProperties?: boolean) {
...
function validate(instance, schema, strictValidation) {
  ...
  Object.keys(flatSchema).forEach(function (name) {
    ...
    if (!(name in flatInstance)) {
      try {
        if (
          typeof schemaItem.default === 'object' &&
        ...
        const err = new Error(
          `configuration param '${name}' missing from config, did you override its parent?`
        );
...
function normalizeSchema(name, node, props, fullName, env, argv, sensitive) {
  if (name === '_cvtProperties') {
    throw new Error(
      `'${fullName}': '_cvtProperties' is reserved word of convict.`
    );
  }
...
function walk(obj, path, initializeMissing?: boolean) {
  if (path) {
    const ar = path.split('.');
    while (ar.length) {
      const k = ar.shift();
  ...
  const rv: any = {
    ...
    set: function (k, v) {
      for (const forbidden_key_path of FORBIDDEN_KEY_PATHS) {
        if (
          k.startsWith(forbidden_key_path) ||
    ...
    validate: function (options) {
      ...
      const output_function = options.output || console.log;
...
convict.addParser = function (parsers) {
  ...
  parsers.forEach(function (parser) {
    ...
    const extensions = !Array.isArray(parser.extension)
      ? [parser.extension]
      : parser.extension;
