# ![logo](/app/public/images/logo.tailchat.svg) App Configuration properties list:

| Name | Description | Format | Default value |
| :--- | :---------- | :----- | :------------ |
 | loginEnabled | 登录是否可用 | `"boolean"` | ``false`` |
| developmentPort | 开发模式监听端口 | `"port"` | ``3443`` |
| productionPort | 生产模式监听端口 | `"port"` | ``443`` |
| serverHostname | If the server component runs on a different host than the app you can specify the host name. | `"string"` | ``""`` |
| supportedBrowsers | Supported browsers version in bowser satisfy format. | `"object"` | ``{  "windows": {    "internet explorer": ">12",    "microsoft edge": ">18"  },  "microsoft edge": ">18",  "safari": ">12",  "firefox": ">=60",  "chrome": ">=74",  "chromium": ">=74",  "opera": ">=62",  "samsung internet for android": ">=11.1.1.52",  "electron": ">=18.1.0"}`` |
...
| defaultLayout | The default layout. | `[  "auto",  "democratic",  "filmstrip"]` | ``"auto"`` |
...
| logo | If not null, it shows the logo loaded from the specified URL, otherwise it shows the title. | `"string"` | ``"/images/logo.tailchat.svg"`` |
| title | The title to show if the logo is not specified. | `"string"` | ``"tailchat"`` |
| supportUrl | The service & Support URL; if `null`, it will be not displayed on the about dialogs. | `"url"` | ``"https://tailchat.msgbyte.com/"`` |
| privacyUrl | The privacy and data protection external URL or local HTML path. | `"string"` | ``"privacy/privacy.html"`` |
| virtualBackground | 虚拟背景图片列表 | `"array"` | ``[  "/images/virtual-background/1.jpg",  "/images/virtual-background/2.jpg",  "/images/virtual-background/3.jpg"]`` |
