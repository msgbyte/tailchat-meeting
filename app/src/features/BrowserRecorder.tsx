...
export default class BrowserRecorder {
  ...
  dest = null;
  gainNode = null;
  audioConsumersMap = new Map();
  micProducerId = null;
  micProducerStreamSource = null;
  ...
  async startLocalRecording({
    ...
    try {
      ...
      if (
        ...
        this.idbName = Date.now().toString();
  ...
  invokeSaveAsDialog(blob: Blob) {
    const link = document.createElement('a');

    link.setAttribute('style', 'display:none;opacity:0;color:transparent;');
  ...
  checkMicProducer(producers) {
    ...
    if (
      ...
      const micProducer: any = Object.values(producers).find(
        (p: any) => p.kind === 'audio'
      );
  ...
  checkAudioConsumer(consumers, recordingConsents?) {
    if (
      ...
      const audioConsumers: any[] = Object.values(consumers).filter(
        (p: any) => p.kind === 'audio' && recordingConsents.includes(p.peerId)
      );
      ...
      for (const key in this.audioConsumersMap.entries()) {
        const [consumerId, aCStreamSource] = key as any;
        if (!audioConsumers.find((c: any) => consumerId === c.id)) {
