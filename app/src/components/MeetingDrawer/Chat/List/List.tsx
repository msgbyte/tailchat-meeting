...
class MessageList extends React.Component<any> {
  refList = React.createRef<HTMLDivElement>();
  currWidth = 0;
  ...
  isMessageSeen = () => {
    ...
    items.forEach((item: any) => {
      ...
      else isSeen = itemRect.top >= listRect.top;
  ...
  render() {
    ...
    if (items.length > 0) {
      if (chat.order === 'asc') items.sort();
      else if (chat.order === 'desc') items.reverse();
    }
        ...
        <div id="chatList" className={classes.root} ref={this.refList}>
          ...
          {items.length === 0 ? (
            ...
            items.map((item, index) => {
              const prev =
                index > 0
                  ? `${items[index - 1].peerId}-${items[index - 1].name}`
                  : null;
              ...
              if (curr !== prev && curr !== next) format = 'single';
              else if (curr !== prev && curr === next) format = 'combinedBegin';
              else if (curr === prev && curr === next)
                format = 'combinedMiddle';
              else if (curr === prev && curr !== next) format = 'combinedEnd';

              if (item.type === 'message') {
                const message = (
                  <Message
                    refMessage={(el) => el && this.setWidth(el)}
                    key={item.time}
                    time={item.time}
                    avatar={
                      (item.sender === 'response' ? item.picture : myPicture) ||
                      EmptyAvatar
                    }
                    name={item.name}
                    text={item.text}
                    isseen={item.isRead}
                    sender={me.id === item.peerId ? 'client' : item.sender}
...
})(withStyles(styles as any)(injectIntl(MessageList)));
