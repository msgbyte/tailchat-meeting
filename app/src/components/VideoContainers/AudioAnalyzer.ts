...
export class AudioAnalyzer {
  ...
  renderScale() {
    ...
    if (!canvasContext || !canvas || !canvas.height) return;
  ...
  render() {
    ...
    if (!canvasContext || !canvas || !canvas.height) return false;
    ...
    for (let i = 0; i < this.frequencyArray!.length; i++) {
      ...
      canvasContext.fillRect(this.canvasColumn, canvas.height * (1 - y), 1, 1);
    }
